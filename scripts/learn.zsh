#!/bin/zsh
#PBS -q default
#PBS -l select=1:ncpus=16:ngpus=1:mem=50gb:host=yayoi18
...
cd /home/ruigengji/L-IPS/2.5 && L_IPS_LIGAND_DPOLAR=-0.15 L_IPS_LIGAND_SCOPE=metal python -u run_zn_job.py --system 1aay --method cwld --ns 5 --seeds 3 > logs/T1_metal_$(date +%H%M%S).log 2>&1; echo "exit=$?"; tail -25 $(ls -t logs/T1_metal_*.log | head -1)
