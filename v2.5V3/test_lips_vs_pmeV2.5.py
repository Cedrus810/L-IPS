...
def build_phase_cwld_metadata(system, topology, dpolar_O=-0.15, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION):
...
def build_cwld_particle_arrays(system, topology, dpolar_O=-0.15, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION):
    meta = build_phase_cwld_metadata(system, topology, dpolar_O, a_q2, enable_water_response, enable_solute_polarization)
...
def build_fast_cwld_metadata(system, topology, dpolar_O=-0.15, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION):
    return build_phase_cwld_metadata(system, topology, dpolar_O, a_q2, enable_water_response, enable_solute_polarization)
...
def compute_water_net_delta_q(traj, meta, ca_atoms, r_env=0.35, rho0=13.5, k_polar=0.8,
    ...
    sampled = triplets
    if len(sampled) > max_triplets_per_frame:
        pick = np.linspace(0, len(sampled) - 1, max_triplets_per_frame, dtype=int)
        sampled = sampled[pick]
    ...
    for frame_idx in range(0, traj.n_frames, frame_stride):
        ...
        for o_idx, h1_idx, h2_idx in sampled:
            net_delta_q = 0.0
...
def setup_cwld_lips_system(base_system, topology, r_env=0.35, rc=1.2, r_on=0.9, rho0=13.5, k_polar=0.8, dpolar_O=-0.15, k_penalty=180.0, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION):
    print("\n" + "="*60)
    print(f"⚙️ [2/4] 正在注入 CWLD-L-IPS 隐式极化引擎 (a_q2={a_q2:.3f}, solute_polarization={enable_solute_polarization}, water_response={enable_water_response})...")
    ...
    meta = build_phase_cwld_metadata(sys_copy, topology, dpolar_O, a_q2, enable_water_response, enable_solute_polarization)
    qbase = meta["qbase"]
    charge_mod_array = meta["charge_mod_array"]
    dpolar = meta["dpolar"]
    is_polar = meta["is_polar"]
...
def setup_cwld_fast_system(base_system, topology, rc=1.2, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION):
    print("\n" + "="*60)
    ...
    meta = build_fast_cwld_metadata(sys_copy, topology, a_q2=a_q2, enable_water_response=enable_water_response, enable_solute_polarization=enable_solute_polarization)
    print(
...
def analyze_1ckk(label, dcd_file, csv_file, topology, system, a_q2=None, force_norm_file=None, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION, q_profile_mode="exact"):
    print(f"\n[📊 分析] 正在提取 {label} 的核心指标...")
    md_top = md.Topology.from_openmm(topology)
    traj = md.load(dcd_file, top=md_top)
