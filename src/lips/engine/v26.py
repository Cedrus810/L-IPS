...
import os, sys, glob, gc, time, warnings, hashlib
import re
...
PROD_STEPS = 250000     # 500 ps；普通消融/快速检查仍保持短轨迹
LONG_CONVERGENCE_PROD_STEPS = 2500000  # 5 ns：Ca-water 动力学收敛验证
...
CHARGE_MOD_MIN = 0.25
CHARGE_MOD_MAX = 2.5
# Sakuraba ZMM (zero-multipole) closure order for the Q–Q electrostatic pair term.
#   ell = 1 -> zero-dipole / RF-like (v2.6 原始行为, U1 = 1/r - 3/(2rc) + r^2/(2rc^3))
#   ell = 2 -> ZQuad (U2, quadrupole cancellation)
#   ell = 3 -> ZOct  (U3, octupole cancellation)
# 全部取 α=0 (damping 关闭，erfc(αr)/r 退化为 1/r)，cutoff rc 由各 setup_* 的 rc 参数控制（默认 1.2 nm）。
# 论文结论：ell=2/3, rc=1.2 nm, α=0 是中高并行下的 fast-simulation 甜点；ZOct 相比调优 SPME 有 16–44% 提速。
# 覆盖方式：环境变量 L_IPS_ZMM_ELL=2 python test_lips_vs_pmeV2.6.py
ZMM_CLOSURE_ELL = int(os.environ.get("L_IPS_ZMM_ELL", "2"))
if ZMM_CLOSURE_ELL not in (1, 2, 3):
    raise ValueError(f"L_IPS_ZMM_ELL 只支持 1/2/3，收到 {ZMM_CLOSURE_ELL}")

# --- closure 家族入口（2026-09-01 加）------------------------------------
#   L_IPS_CLOSURE=zmm     (默认) 走下面 zmm_uclosure() 的冻结字符串，语义一字不变，
#                         并继续读 L_IPS_ZMM_ELL ∈ {1,2,3}
#   L_IPS_CLOSURE=pswfz2  带限最优窗（PSWF 判据 + ell=2 端点约束，带宽 c=2π 由 rc 定死）
#                         实测：带外能量比 zmm2 低 13.7 倍，S_ZZ 加权力误差高 16.4%
# 设计与判据见 PLAN_PSWF_Closure_Family.md / STAGE1_CLOSURE_KILLSWITCH_REPORT.md。
# 纪律：默认路径**不导入窗层、不改一个字节**，新窗才走 lips.engine.closure。
L_IPS_CLOSURE = os.environ.get("L_IPS_CLOSURE", "zmm").strip().lower()
_SUPPORTED_CLOSURES = ("zmm", "pswfz2")
if L_IPS_CLOSURE not in _SUPPORTED_CLOSURES:
    raise ValueError(f"L_IPS_CLOSURE 只支持 {_SUPPORTED_CLOSURES}，收到 {L_IPS_CLOSURE!r}")

# --- 子集运行 + 产出打标签（2026-09-01 加，为「同 closure 重比」服务）----------
#   L_IPS_ONLY_CONFIGS=CWLD_aq0p5_ca_w2      只跑这些 config（逗号分隔），默认全跑
#   L_IPS_RUN_TAG=ell1                       给 run label 和汇总 csv 加后缀
# 后者是必需的：run label 决定 dcd/csv 文件名，而文件名里本来不带 closure 标识，
# 换个 closure 再跑一次就会**直接覆盖**上一轮的产出（2026-09-01 差点因此丢掉 18 小时）。
L_IPS_ONLY_CONFIGS = [c.strip() for c in
                      os.environ.get("L_IPS_ONLY_CONFIGS", "").split(",") if c.strip()]
_raw_tag = os.environ.get("L_IPS_RUN_TAG", "").strip()
if _raw_tag and not re.fullmatch(r"[A-Za-z0-9_.-]+", _raw_tag):
    raise ValueError(f"L_IPS_RUN_TAG 只允许字母数字和 _.- ，收到 {_raw_tag!r}")
L_IPS_RUN_TAG = f"_{_raw_tag}" if _raw_tag else ""


def zmm_uclosure(ell=None, rc="rc"):
    """Lepton 表达式：Sakuraba 零多极 (α=0) closure U_ell(r)。

    U_ell(r) = 1/r + sum_{k=0}^{ell} c_k r^{2k}，系数 c_k 选到让
    (d^m U / dr^m)|_{r=rc} = 0, m = 0..ell，因此能量和力在 cutoff 处都平滑归零。
    返回的字符串用符号 `r` 和给定的 `rc`（默认全局参数名 "rc"）。
    """
    if ell is None:
        ell = ZMM_CLOSURE_ELL
    if ell == 1:
        return (f"(1.0/r - 1.5/{rc} + r^2/(2.0*{rc}^3))")
    if ell == 2:
        return (f"(1.0/r - 15.0/(8.0*{rc}) + 5.0*r^2/(4.0*{rc}^3)"
                f" - 3.0*r^4/(8.0*{rc}^5))")
    if ell == 3:
        return (f"(1.0/r - 35.0/(16.0*{rc}) + 35.0*r^2/(16.0*{rc}^3)"
                f" - 21.0*r^4/(16.0*{rc}^5) + 5.0*r^6/(16.0*{rc}^7))")
    raise ValueError(f"unsupported ZMM closure order ell={ell}")
def closure_label():
    """日志/CSV 里用的完整 closure 标识（窗名 + 全部参数）。"""
    if L_IPS_CLOSURE == "zmm":
        name = {1: "ZDipole/RF-like", 2: "ZQuad", 3: "ZOct"}[ZMM_CLOSURE_ELL]
        return f"zmm{ZMM_CLOSURE_ELL}({name}, alpha=0)"
    return "pswfz2(band-limited, ell=2, c=2pi)"


def pair_uclosure(rc="rc"):
    """按 L_IPS_CLOSURE 分派 pair closure 的 Lepton 表达式。

    zmm 分支**原样返回 zmm_uclosure() 的冻结字符串**——不经窗层重新生成，
    避免 Lepton 浮点求值顺序变化让 1e-12 的 fixture 断言变红。
    """
    if L_IPS_CLOSURE == "zmm":
        return zmm_uclosure(rc=rc)
    from lips.engine import closure as _cw   # 只在非默认路径导入
    return _cw.make_window(L_IPS_CLOSURE).lepton(rc)
...
# 长轨迹必须监控势能的平稳性。注意：Langevin/NVT 下势能不是守恒量，
# half-shift/slope 用来诊断非平稳漂移，不等同于严格的 NVE 能量守恒测试。
...
#   L_IPS_RUN_MODE=long_convergence         python test_lips_vs_pmeV2.6.py
# 不设置该环境变量时，完全按上面几个布尔值手动控制，行为跟不加这个开关之前一致。
RUN_MODE = os.environ.get("L_IPS_RUN_MODE", "").strip().lower()
_VALID_RUN_MODES = {"mts_validation", "polarization_ablation", "ca_source_weight_sweep", "profile_force_cost", "default_ablation", "replicate_matrix", "long_convergence"}
if RUN_MODE:
    ...
    RUN_LONG_CONVERGENCE = (RUN_MODE == "long_convergence")
...
# --- 逐原子 dpolar 覆盖（2026-09-02 加）---------------------------------------
# 为什么需要：上面两张表是**按元素**的，而同一个残基里不同的氮化学上完全不同。
# 实测 1AAY：HID125 的 NE2（去质子、配位 Zn）与 ND1（带 H、朝外）拿到同一个
# dpolar=0.012，全靠局部密度碰巧把它们分开（NE2 dens=2.12 / ND1 dens=0.00）——
# 那是几何巧合，不是化学识别。而蛋白里 152 个氮里只有 6 个是配位的（3.9%）。
#
# 两道被实测确认的天花板：
#   (a) 幅度：即使 tanh 饱和到 1，Δq 上限就是 dpolar 本身 = 0.012 e = clamp(0.2) 的 6%。
#       `Q_DELTA_CLAMP` 对溶质从来没起作用。对比水 dpolar_O = -0.15 = clamp 的 75%。
#   (b) 符号：溶质 dpolar 全为正 ⇒ Δq>=0 ⇒ 配位 N/S 只会变正，即**削弱**对金属阳离子
#       的吸引；而水是负的（O 更负、配位更强）。在 1AAY 上 His 正是塌掉的那一侧。
#
# 键为 (残基名, 原子名)。判据是**化学**（去质子的孤对电子给体），不是"是否靠近金属"，
# 所以不需要几何检测；HIE 的 ND1 即使这一结构里没配位金属，化学上同样是那个氮。
# 默认**关闭**（空表）以保持 1CKK 既有结果可复现；用环境变量显式开启并给幅度：
#   L_IPS_LIGAND_DPOLAR=-0.15 python test_lips_vs_pmeV2.6.py
# 作用范围开关：L_IPS_LIGAND_SCOPE
#   metal (默认) 只作用于金属第一壳的去质子给体 —— CYM/CYD 的 SG、HID 的 NE2、HIE 的 ND1
#   all         再加上 Asp/Glu 的羧基氧（1CKK 的 Ca 配体）
# 为什么要分：1AAY 里 Asp/Glu 的羧基氧**不配位 Zn**，但按残基名会全部命中（实测 18 个，
# 而 Zn 相关只有 13 个）。它们会改变整个蛋白的静电与构象，混进来就没法把改善归因到
# "金属配位原子的极化"这件事上。
_LIGAND_METAL_ATOMS = (("HID", "NE2"), ("HIE", "ND1"), ("CYM", "SG"), ("CYD", "SG"))
_LIGAND_CARBOXYL_ATOMS = (("ASP", "OD1"), ("ASP", "OD2"),
                          ("GLU", "OE1"), ("GLU", "OE2"))
LIGAND_DPOLAR_ATOMS = _LIGAND_METAL_ATOMS + _LIGAND_CARBOXYL_ATOMS

_lig_dp = os.environ.get("L_IPS_LIGAND_DPOLAR", "").strip()
LIGAND_DPOLAR_VALUE = float(_lig_dp) if _lig_dp else None
LIGAND_DPOLAR_SCOPE = os.environ.get("L_IPS_LIGAND_SCOPE", "metal").strip().lower()
if LIGAND_DPOLAR_SCOPE not in ("metal", "all"):
    raise ValueError(f"L_IPS_LIGAND_SCOPE 只支持 metal / all，收到 {LIGAND_DPOLAR_SCOPE!r}")
_scoped = (_LIGAND_METAL_ATOMS if LIGAND_DPOLAR_SCOPE == "metal"
           else LIGAND_DPOLAR_ATOMS)
LIGAND_DPOLAR_OVERRIDE = ({k: LIGAND_DPOLAR_VALUE for k in _scoped}
                          if LIGAND_DPOLAR_VALUE is not None else {})
CHARGED_SIDECHAIN_POLAR_ATOMS = {
    ...
    "GLU": {"OE1", "OE2"},
    "HIS": {"ND1", "NE2"},
    # --- 2026-09-01 追加：金属配位残基的质子化态变体名 ---------------------
    # 命中这个字典的原子会拿 CHARGED_SOURCE_WEIGHT(1.5) 而不是 POLAR_SOURCE_WEIGHT(1.0)，
    # 也就是作为密度源的权重差 1.5 倍。原表只有 "HIS"：1CKK 走
    # `PDBFixer.addMissingHydrogens()`，残基名保持 "HIS"，所以那边一直是命中的（**非老 bug**）。
    # 但 1AAY 走 `Modeller.addHydrogens(variants=['HID',...])`，残基会被**改名**成
    # HID/HIE，就落不进原表；去质子 Cys 更是完全没有条目。
    # 1AAY 的 Zn 位点正是 Cys₂His₂ —— 不补这几条，最该被极化的那 12 个配位原子
    # 会被当成普通极性原子，作为密度源少 33% 的权重，等于在要看的位点上自我削弱。
    "HID": {"ND1", "NE2"},      # 配位 NE2、H 在 ND1（1AAY 的 6 个 His 全是这个）
    "HIE": {"ND1", "NE2"},
    "HIP": {"ND1", "NE2"},      # 质子化组氨酸，+1
    "CYM": {"SG"},              # 去质子硫醇盐，−1；SG 电荷 −0.8844
    "CYD": {"SG"},              # 同上，AMOEBA/Tinker 的叫法
}
...
def wrap_into_box(xyz, box):
    """np.mod(xyz, box) into [0, box), guarding the float-rounding edge case
    where the result lands exactly on (or a hair above) box due to how mod is
    computed -- cKDTree(boxsize=...) rejects any coordinate >= box."""
    coords = np.mod(xyz, box)
    return np.where(coords < box, coords, 0.0)
...
def build_phase_cwld_metadata(system, topology, dpolar_O=-0.15, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION, ca_source_weight=CA_SOURCE_WEIGHT):
        ...
        if residue_name in WATER_RESNAMES:
            ...
            if enable_solute_polarization:
                for atom in residue.atoms():
                    ...
                    _ov = LIGAND_DPOLAR_OVERRIDE.get((residue_name, atom.name))
                    if _ov is not None:                 # 逐原子覆盖元素规则
                        dpolar[atom.index] = _ov
                        is_polar[atom.index] = 1.0
                        ligand_override_indices.append(atom.index)
    ...
    return {
        ...
        "dpolar": dpolar,
        "is_polar": is_polar,
        "ligand_override_indices": np.array(ligand_override_indices, dtype=int),
...
def compute_fast_cwld_q_from_positions(positions_nm, box_vectors_nm, meta, a_q2, r_env=0.35, rho0=13.5, k_polar=0.8):
    ...
    coords = wrap_into_box(positions_nm, box_lengths) if has_periodic_box else positions_nm
    ...
    for frame_idx in frame_indices:
        ...
        if len(sampled_waters) > max_per_frame:
            if ca_tree is not None:
                ca_dist_to_water, _ = ca_tree.query(coords[water_o_indices], k=1)
                near_mask = ca_dist_to_water <= NEAR_CA_CUTOFF_NM
            else:
                near_mask = np.zeros(len(sampled_waters), dtype=bool)
            # Force-keep every water within the Ca first shell so downstream
            # ca_near_water_* stats never silently drop to an empty sample;
            # only the remaining bulk-water budget gets linspace-subsampled.
            near_waters = [w for w, keep in zip(sampled_waters, near_mask) if keep]
            far_waters = [w for w, keep in zip(sampled_waters, near_mask) if not keep]
            remaining_budget = max(0, max_per_frame - len(near_waters))
            if far_waters and len(far_waters) > remaining_budget:
                pick = np.linspace(0, len(far_waters) - 1, remaining_budget, dtype=int) if remaining_budget > 0 else np.array([], dtype=int)
                far_sampled = [far_waters[i] for i in pick]
            else:
                far_sampled = far_waters
            sampled_waters = near_waters + far_sampled
...
def compute_atom_delta_q_profile(traj, meta, atom_indices, r_env=0.35, rho0=13.5, k_polar=0.8,
    ...
    for frame_idx in range(0, traj.n_frames, frame_stride):
        ...
        coords = wrap_into_box(xyz, box) if has_box else xyz
...
# ==========================================
# LocalCWLDForce 插件：生产路径的 CWLD 引擎（LCWLD-161 起）
# ==========================================
# 插件比 CustomGBForce 快 3.1-3.6x（同体系同积分器，1AAY 32794 原子：
# 54.78 -> 184.33 ns/day）。数值上等价但**不是逐比特相同** —— 逐原子力
# p99 3.3e-3 对阈值 1.0。所以两个引擎的轨迹不能混进同一批统计里，
# 这正是 2.4 那次 5ns 矩阵 closure 不一致踩过的坑。
#
# 因此：`engine` 被记进每个 System 的名字里（见 setup_cwld_lips_system），
# 补跑旧矩阵必须显式传 engine="customgb"。
#
# 插件从 repo 内的构建树加载，**不装进 conda env** —— 那个 env 是共享的，
# 往它的 plugin 目录里放东西会让机器上所有 OpenMM 进程都加载这个力。
_LOCALCWLD_ROOT = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))))), "openmm-localcwld")
_localcwld_loaded = False


def ensure_localcwld_plugin():
    """幂等地加载 LocalCWLDForce 插件，并让 openmm_localcwld 可导入。

    找不到就抛错，不静默退回 CustomGBForce：静默退回意味着一批本该是插件
    引擎的结果混进了旧引擎，而那正是这里要防的事。
    """
    global _localcwld_loaded
    if _localcwld_loaded:
        return
    # 一棵构建树只能属于一台机器：CMakeCache 里存的是编译器/工具链的绝对路径。
    # 这个仓库在 NFS 上被多台机器挂着，共用一个 build 目录会让后一台的 cmake
    # 拿着前一台的配置去建，失败并且删掉前一台已建好的产物（2026-09-05 实际发生过）。
    # 所以每台机器一个目录，用 LOCALCWLD_BUILD_DIR 指定，默认本机那棵。
    build_dir = os.environ.get("LOCALCWLD_BUILD_DIR", "build-cuda")
    plugin_dir = os.path.join(_LOCALCWLD_ROOT, build_dir, "platforms", "cuda")
    if not os.path.isdir(plugin_dir):
        raise RuntimeError(
            f"LocalCWLDForce 插件没有构建：{plugin_dir} 不存在。\n"
            f"  cd {_LOCALCWLD_ROOT} && cmake --build {build_dir} -j\n"
            f"（别的机器用 LOCALCWLD_BUILD_DIR 指到自己那棵树）")
    # The kernel reads OpenMM's INTERNAL neighbour-list layout -- exclusionTiles,
    # interactingAtoms, interactionCount -- which is not in the installed headers
    # (see the traversal notes in platforms/common/kernels/localCWLD.cc). It was
    # transcribed from this exact revision. A different OpenMM would not fail to
    # compile or to load; it would silently compute wrong forces, which is the
    # one failure mode worth a runtime check.
    # ⚠ Do NOT "fix" this environment by upgrading OpenMM. What mamba currently
    # offers as 8.6.0 is an ALPHA, not a release, and that alpha build has no GPU
    # support -- unusable for production here (user, 2026-09-05). The pin on
    # 8.5.2.dev-36a30cb is a choice, not staleness. Anything claiming this
    # machine runs 8.6.0 -- sibling projects' ENVIRONMENT.md included -- is
    # wrong; ask the interpreter, not a document.
    #
    # This is about the pre-release, not about 8.6 forever: once a real 8.6
    # release ships with GPU support, upgrading can be reconsidered -- but it
    # still has to clear the neighbour-list layout question below.
    built_against = "36a30cbca54e727b216b606f3c011b67201eb8b4"
    running = getattr(mm.version, "git_revision", "") or "unknown"
    if running != built_against:
        warnings.warn(
            f"LocalCWLDForce 的 kernel 是照着 OpenMM {built_against[:7]} 的内部邻居表"
            f"布局写的，当前跑的是 {running[:7]}（{mm.version.full_version}）。"
            f"布局若已改动不会报错，只会静默算错力 —— 先跑一遍插件的 ctest"
            f"（尤其 TestCudaLocalCWLDAcceptance，它逐原子对生产 CustomGBForce）"
            f"再信任结果。另：mamba 上的 8.6.0 是 alpha 预发布、没有 GPU 支持，别往那儿升。",
            RuntimeWarning, stacklevel=2)

    loaded = mm.Platform.loadPluginsFromDirectory(plugin_dir)
    if not any("LocalCWLDCUDA" in name for name in loaded):
        raise RuntimeError(
            f"在 {plugin_dir} 里没有加载到 LocalCWLDCUDA。"
            f"加载到的是：{loaded}。失败原因见 Platform.getPluginLoadFailures()")
    # Two paths: the pure-Python helpers (metadata, xml_bridge) and the SWIG
    # extension. The latter is the only way to recognise the force again --
    # after deserialisation System.getForce() hands back a generic Force, so a
    # plain isinstance never matches and type(f).__name__ is just "Force".
    for pypkg in (os.path.join(_LOCALCWLD_ROOT, "python"),
                  os.path.join(_LOCALCWLD_ROOT, "build-cuda", "python")):
        if pypkg not in sys.path:
            sys.path.insert(0, pypkg)
    _localcwld_loaded = True


def _swap_to_localcwld(system, *, r_env, rc, rho0, k_polar,
                       q_clamp, use_penalty, penalty_k, zmm_ell):
    """把 System 里那个 CustomGBForce 原地换成等价的 LocalCWLDForce。

    走 xml_bridge 而不是手写映射：那条路的列映射由 LCWLD-060/080 的互证
    测试守着，手写一遍等于把守卫绕开。

    ⚠⚠ **2026-09-07 修了一个静默算错力的 bug。** 原来这个函数调
    `swap_customgb_for_localcwld(xml)` **不传 `globals_`**，于是桥用它自己的
    `DEFAULT_GLOBALS` —— 里面 `zmmOrder: 2` 与全部引擎参数都是**硬编码**的，
    而桥**从不读 CustomGBForce 的 pair 表达式**去反推 closure。后果（实测）：

        L_IPS_ZMM_ELL=1, engine="plugin"
          -> 日志/CSV 报 zmm1，插件里实际 ZMMOrder = 2

    影响面不止 closure：`r_env` / `rc` / `rho0` / `k_polar` / `clamp` / penalty
    **在插件路径上全部被静默钉在默认值**，也就是任何参数扫描都无效。
    至今没炸只是因为所有生产跑恰好用的就是那套默认值，而 DEC-005 的验收
    **也是在默认参数下做的**，结构上抓不到它。

    现在把真实参数显式传下去，并对**插件表达不了的 closure 直接抛错**
    （插件只有整数 `zmmOrder` 1/2/3；PCF/pswfz2 需要 PLAN §14.7b 的系数入口）。
    回归：`tests/test_plugin_globals.py`。
    """
    if L_IPS_CLOSURE != "zmm":
        raise RuntimeError(
            f"engine='plugin' 表达不了 closure={closure_label()}：\n"
            f"  插件的 API 只有整数 zmmOrder（1/2/3），非 ZMM 窗需要\n"
            f"  PLAN_PSWF_Closure_Family.md §14.7b 的「4 个偶次系数」入口（尚未实现）。\n"
            f"  ⚠ 不要改用 engine='customgb' 后与插件数据混进同一统计（DEC-005）。")
    ensure_localcwld_plugin()
    ...
    return mm.XmlSerializer.deserialize(
        swap_customgb_for_localcwld(
            mm.XmlSerializer.serialize(system),
            globals_=dict(
                environmentCutoff=r_env,
                cutoffDistance=rc,
                zmmOrder=int(zmm_ell),
                rho0=rho0,
                kPolar=k_polar,
                chargeDeltaClamp=q_clamp,
                useQPenalty=bool(use_penalty),
                qPenaltyStrength=penalty_k,
            )))


def setup_cwld_lips_system(base_system, topology, *, engine="plugin", r_env=0.35, rc=1.2, r_on=0.9, rho0=13.5, k_polar=0.8, dpolar_O=-0.15, k_penalty=180.0, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION, ca_source_weight=CA_SOURCE_WEIGHT):
    if engine not in ("plugin", "customgb"):
        raise ValueError(f"engine 只能是 'plugin' 或 'customgb'，收到 {engine!r}")
    print("\n" + "="*60)
    ...
    if LIGAND_DPOLAR_VALUE is not None:
        print(f"  -> L_IPS_LIGAND_DPOLAR={LIGAND_DPOLAR_VALUE:+.4f} "
              f"scope={LIGAND_DPOLAR_SCOPE}：配位原子 dpolar 覆盖已开启")
    else:
        print(f"  -> L_IPS_LIGAND_DPOLAR 未设：配位原子沿用按元素的 dpolar（1CKK 口径）")
    ...
    # 净静电 = Q1*Q2 * U_ell(r)（Sakuraba ZMM 零多极 closure，α=0）。
    # 减去 qbase1*qbase2*(1/r - 1/rc) 抵消原生 NonbondedForce 的 reaction-field 静电
    # (ε_RF=1 -> 1/r - 1/rc)，这一项与 closure 阶数无关，永远保持 1/r - 1/rc。
    print(f"  -> pair closure: {closure_label()}, rc={rc} nm")
    pair_expr = f"""
    ONE_4PI_EPS0 * (
        Q1*Q2 * {pair_uclosure()}
      - qbase1*qbase2 * (1.0/r - 1.0/rc)
    );
    """
    ...
    # LCWLD-161. `plugin` is the production path: same physics, 3.1-3.6x faster.
    # `customgb` is kept for topping up the pre-2026-09-05 matrices -- the two
    # engines agree to p99 3.3e-3 per-atom force but are not bitwise identical,
    # so a single statistic must never mix them.
    if engine == "plugin":
        sys_copy = _swap_to_localcwld(
            sys_copy, r_env=r_env, rc=rc, rho0=rho0, k_polar=k_polar,
            q_clamp=Q_DELTA_CLAMP, use_penalty=ENABLE_Q_PENALTY,
            penalty_k=k_penalty, zmm_ell=ZMM_CLOSURE_ELL)
        print(f"  -> CWLD 引擎：LocalCWLDForce 插件（生产路径），"
              f"zmmOrder={ZMM_CLOSURE_ELL} 已显式传入")
    else:
        print("  -> CWLD 引擎：CustomGBForce（旧引擎；只用于补跑既有矩阵）")
    return sys_copy


def cwld_engine_of(system):
    """报告某个 System 用的是哪个 CWLD 引擎。产物落盘时记这个。

    半年后没人能从一个 dcd 看出它是哪个引擎跑的 —— 2.4 的 5ns 矩阵就是这么
    把两批数混掉的。所以每个产物都该带上这一栏。
    """
    forces = list(system.getForces())
    if any(isinstance(f, mm.CustomGBForce) for f in forces):
        return "customgb"
    try:
        ensure_localcwld_plugin()
        import localcwld
        ...
        return "none"
    if any(localcwld.LocalCWLDForce.isinstance(f) for f in forces):
        return "plugin"
    return "none"
...
def setup_cwld_fast_system(base_system, topology, rc=1.2, a_q2=DEFAULT_A_Q2, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION, ca_source_weight=CA_SOURCE_WEIGHT):
    ...
    # 与 exact 引擎保持同一 ZMM closure：净静电 = Q1*Q2 * U_ell(r)，
    # 减去 qbase1*qbase2*(1/r - 1/rc) 抵消原生 reaction-field (ε_RF=1)。
    expr = f"""
    ONE_4PI_EPS0 * (
        Q1*Q2 * {pair_uclosure()}
      - qbase1*qbase2 * (1.0/r - 1.0/rc)
...
def select_platform(verbose=True):
    """硬件加速：CUDA 是唯一受支持的生产 platform，缺了就直接失败。

    历史：v2.5 是 bare except 静默摔到 CPU（日志里看不出掉速）；v2.6 改成
    CUDA -> OpenCL -> CPU 分级回退并打印实际 platform。但回退本身仍然有害——
    2026-08-31 换服务器后目标环境固定为单机 1x CUDA GPU（见
    `openmm-localcwld/docs/decisions/DEC-003-platform-scope.md`），一条 5 ns
    replicate 轨迹静默掉到 CPU 意味着几天白跑却还产出"看起来正常"的 csv。

    所以现在：默认 **硬要求 CUDA**，拿不到就 raise。需要在没有 GPU 的机器上
    做小规模调试时，显式设 `L_IPS_PLATFORM=cpu`（或 opencl / reference）来
    opt-in——非 CUDA 时会打印醒目警告，其结果不得当成生产数据。
    """
    requested = os.environ.get("L_IPS_PLATFORM", "cuda").strip().lower()
    canonical = {"cuda": "CUDA", "opencl": "OpenCL", "cpu": "CPU", "reference": "Reference"}
    if requested not in canonical:
        raise ValueError(
            f"L_IPS_PLATFORM={requested!r} 不认识，可选 {sorted(canonical)}（默认 cuda）"
        )
    ...
    try:
        ...
        if name == "CUDA":
            raise RuntimeError(
                f"❌ 拿不到 CUDA platform（{e}）。这个项目此后只在 CUDA 上跑生产轨迹，"
                f"不会静默退回 CPU。当前 OpenMM 报告的 platform: {available}。\n"
                f"   先排查：conda env 是不是 /home/ruigengji/miniforge3/envs/openmm_dev、"
                f"nvidia-smi 是否正常、CUDA_VISIBLE_DEVICES 是否被设成空。\n"
                f"   确实想在无 GPU 机器上做小规模调试，显式设 L_IPS_PLATFORM=cpu。"
            ) from e
        raise RuntimeError(
            f"❌ 显式请求的 platform {name} 不可用（{e}）。当前可用: {available}。"
        ) from e
    ...
    if verbose:
        ...
        if name == "CUDA":
            ...
            print(f"  -> 🚨 非 CUDA platform（{name}）是通过 L_IPS_PLATFORM 显式 opt-in 的，"
                  f"仅供调试，产出的轨迹/指标不要当生产数据用。")
...
def run_multistage_md(label, system, topology, positions, extra_reporters=None, mts_ratio=None,
                      seed=None, prod_steps_override=None):
    print(f"\n[🚀 模拟] 启动 {label} 多阶段平行试验...")
    ...
    requested_prod_steps = PROD_STEPS if prod_steps_override is None else prod_steps_override
    prod_steps = scaled_step_count(requested_prod_steps, integrator)
    ...
    # FastQUpdateReporter 一类 reporter 会参与动力学，必须覆盖平衡阶段；轨迹、状态和
    # ForceNorm 诊断则只记录正式 production，避免把前 100 ps（含蛋白限制）混进 RDF/ACF。
    if extra_reporters:
        sim.reporters.extend(extra_reporters)
...
def analyze_1ckk(label, dcd_file, csv_file, topology, system, a_q2=None, force_norm_file=None, enable_water_response=ENABLE_WATER_RESPONSE, enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION, q_profile_mode="exact", ca_source_weight=CA_SOURCE_WEIGHT):
    ...
    state_metrics = {
        ...
        "potential_energy_slope_kj_mol_ns": np.nan,
    }
    if os.path.exists(csv_file):
        ...
        if pe_col is not None:
            ...
            pe_time_col = column_containing("time")
            if pe_time_col is not None and len(pe_values) >= 3:
                pe_times_ps = pd.to_numeric(state_df[pe_time_col], errors="coerce").dropna().to_numpy()
                n_fit = min(len(pe_values), len(pe_times_ps))
                if n_fit >= 3 and np.ptp(pe_times_ps[:n_fit]) > 0:
                    slope_per_ps = np.polyfit(pe_times_ps[:n_fit], pe_values[:n_fit], 1)[0]
                    state_metrics["potential_energy_slope_kj_mol_ns"] = float(1000.0 * slope_per_ps)
    ...
    def calc_ca_water_residence(cutoff_nm=0.30):
        ...
        tail_start = max(1, int(0.9 * len(acf)))
        acf_tail_mean = float(np.nanmean(acf[tail_start:])) if tail_start < len(acf) else np.nan
        return (
            float(tau_ps),
            float(t_1e_ps) if np.isfinite(t_1e_ps) else np.nan,
            avg_shell_waters,
            acf_tail_mean,
            bool(np.isfinite(t_1e_ps)),
        )

    (ca_water_residence_tau_ps,
     ca_water_residence_t_1e_ps,
     ca_avg_shell_waters_0p30,
     ca_water_residence_acf_tail_mean,
     ca_water_residence_t_1e_observed) = calc_ca_water_residence(0.30)
    ...
    if positive_n_atoms and negative_o_atoms:
        ...
        salt_first_peak_auc = rdf_auc(r_salt, gr_salt, 0.20, 0.45)
        salt_distances = md.compute_distances(traj, salt_pairs, periodic=True)
        salt_contacts_per_frame = np.sum(salt_distances < 0.40, axis=1)
        salt_contact_count_mean = float(np.mean(salt_contacts_per_frame))
        salt_contact_frame_fraction = float(np.mean(salt_contacts_per_frame > 0))
    else:
        r_salt, gr_salt = np.array([]), np.array([])
        salt_peak = np.nan
        salt_peak_r = np.nan
        salt_first_peak_auc = np.nan
        salt_contact_count_mean = np.nan
        salt_contact_frame_fraction = np.nan
    ...
    metrics = {
        ...
        **force_norm_metrics,
        ...
        "ca_water_residence_t_1e_observed": ca_water_residence_t_1e_observed,
        "ca_water_residence_acf_tail_mean": ca_water_residence_acf_tail_mean,
        "ca_water_residence_window_ps": float(times_ps[-1] - times_ps[0]) if len(times_ps) > 1 else 0.0,
        ...
        "salt_bridge_first_peak_auc_0p20_0p45": salt_first_peak_auc,
        "salt_bridge_contact_count_mean_0p40_nm": salt_contact_count_mean,
        "salt_bridge_contact_frame_fraction_0p40_nm": salt_contact_frame_fraction,
...
def main():
    active_modes = [
        name for name, flag in (
            ...
            ("long_convergence", RUN_LONG_CONVERGENCE),
        ) if flag
    ] or ["default_ablation"]
    print("\n" + "#"*60)
    ...
    if RUN_LONG_CONVERGENCE:
        # 5 ns production-only 收敛验证：保留 PME、ca_w=1、ca_w=2 三个端点，
        # 外加 aq0.5 charge-weighted 默认配方（v2.6_results_analysis.md 第3+4/5节里唯一
        # 还没有做 replicate 验证、但看起来最像 PME 又同时 peak_g 偏离最大的那组）。
        # 使用独立标签，绝不复用/覆盖之前含平衡帧的 500 ps replicate 文件。
        if CWLD_ENGINE != "customgb_exact":
            raise ValueError("RUN_LONG_CONVERGENCE requires CWLD_ENGINE='customgb_exact'.")
        ...
        long_configs = {
            ...
            "CWLD_aq0_ca_w1": {
                "label": "LONG5ns_CWLD_aq0_ca_w1",
                "system": setup_cwld_lips_system(
                    sys_pme, topology, a_q2=PURE_DENSITY_A_Q2,
                    enable_water_response=True,
                    enable_solute_polarization=ENABLE_SOLUTE_POLARIZATION,
                    ca_source_weight=1.0,
                ),
                "a_q2": PURE_DENSITY_A_Q2,
                "enable_water_response": True,
                "enable_solute_polarization": ENABLE_SOLUTE_POLARIZATION,
            ...
            "CWLD_aq0p5_ca_w2": {
                "label": "LONG5ns_CWLD_aq0p5_ca_w2",
                "system": setup_cwld_lips_system(
        ...
        # --- 子集过滤与打标签（见 L_IPS_ONLY_CONFIGS / L_IPS_RUN_TAG）---
        if L_IPS_ONLY_CONFIGS:
            unknown = set(L_IPS_ONLY_CONFIGS) - set(long_configs)
            if unknown:
                raise ValueError(f"L_IPS_ONLY_CONFIGS 里有未知 config {sorted(unknown)}；"
                                 f"可选: {sorted(long_configs)}")
            long_configs = {k: v for k, v in long_configs.items()
                            if k in L_IPS_ONLY_CONFIGS}
            print(f"  -> L_IPS_ONLY_CONFIGS 生效，只跑: {sorted(long_configs)}")
        if L_IPS_RUN_TAG:
            for v in long_configs.values():
                v["label"] = v["label"] + L_IPS_RUN_TAG
            print(f"  -> L_IPS_RUN_TAG 生效，产出后缀: {L_IPS_RUN_TAG}"
                  f"（closure={closure_label()}）")

        production_ns = LONG_CONVERGENCE_PROD_STEPS * DT.value_in_unit(unit.picoseconds) / 1000.0
        equilibration_ps = EQ_STEPS * DT.value_in_unit(unit.picoseconds)
        report_interval_ps = REPORT_INTERVAL * DT.value_in_unit(unit.picoseconds)
        with open(__file__, "rb") as script_handle:
            script_sha256 = hashlib.sha256(script_handle.read()).hexdigest()

        # 增量续跑：已经跑过并落盘的 config（按 per-seed CSV 里的 config 列判断）直接跳过，
        # 不重跑、不覆盖，只补齐 long_configs 里新增的 config，再 append 进已有文件。
        long_detail_file = f"1ckk_v26_long_convergence_5ns_per_seed_metrics{L_IPS_RUN_TAG}.csv"
        long_summary_file = f"1ckk_v26_long_convergence_5ns_summary{L_IPS_RUN_TAG}.csv"
        existing_long_df = pd.read_csv(long_detail_file) if os.path.exists(long_detail_file) else None
        done_configs = set(existing_long_df["config"].unique()) if existing_long_df is not None else set()
        pending_configs = {name: cfg for name, cfg in long_configs.items() if name not in done_configs}
        if done_configs:
            print(f"  -> {long_detail_file} 已存在，跳过已完成 config: {sorted(done_configs)}；"
                  f"本次只跑: {sorted(pending_configs) if pending_configs else '（无，全部已完成）'}")

        print(f"  -> 长轨迹矩阵: {len(pending_configs)} configs x {len(REPLICATE_SEEDS)} seeds; "
              f"每条 production={production_ns:g} ns，只记录无约束 production。")

        long_rows = []
        for name, cfg in pending_configs.items():
            for seed in REPLICATE_SEEDS:
                run_label = f"{cfg['label']}_seed{seed}"
                dcd_file, csv_file = run_multistage_md(
                    run_label, cfg["system"], topology, positions, seed=seed,
                    prod_steps_override=LONG_CONVERGENCE_PROD_STEPS,
                )
                ...
                row["config"] = name
                row["seed"] = seed
                row["production_ns"] = production_ns
                row["equilibration_ps"] = equilibration_ps
                row["report_interval_ps"] = report_interval_ps
                row["analysis_scope"] = "production_only"
                row["protocol_version"] = "v2.6-long-convergence-5ns-v1"
                row["script_sha256"] = script_sha256
                row["a_q2"] = np.nan if cfg["a_q2"] is None else cfg["a_q2"]
                long_rows.append(row)

        new_long_df = pd.DataFrame(long_rows)
        if existing_long_df is not None:
            long_df = pd.concat([existing_long_df, new_long_df], ignore_index=True) if not new_long_df.empty else existing_long_df
        else:
            long_df = new_long_df
        long_df.to_csv(long_detail_file, index=False)

        numeric_cols = long_df.select_dtypes(include=[np.number]).columns.drop("seed", errors="ignore")
        long_summary = long_df.groupby("config")[numeric_cols].agg(["count", "mean", "std"])
        long_summary.columns = [f"{col}_{stat}" for col, stat in long_summary.columns]
        long_summary.to_csv(long_summary_file)

        focus_metrics = [
            "ca_water_peak_g_mean", "ca_water_coord_0p30_nm_mean",
            "ca_protein_o_coord_0p30_nm_mean", "ca_total_o_coord_0p30_nm_mean",
            "ca_water_first_peak_auc_0p20_0p32_mean",
            "ca_water_residence_tau_ps_mean", "ca_water_residence_t_1e_ps_count",
            "ca_water_residence_acf_tail_mean_mean",
            "salt_bridge_contact_frame_fraction_0p40_nm_mean",
            "potential_energy_drift_kj_mol_mean", "potential_energy_slope_kj_mol_ns_mean",
        ]
        print("\n" + "="*72)
        print("📊 5 ns Ca-water 长轨迹收敛结果（注意 water/protein/total coordination 已分列）:")
        print(long_summary[[c for c in focus_metrics if c in long_summary.columns]].to_string(
            float_format=lambda x: f"{x:.4f}"))
        print(f"  -> ✅ 逐 seed: {long_detail_file}")
        print(f"  -> ✅ count/mean/std 汇总: {long_summary_file}")
        print("  -> t_1e count < 3 表示存在右删失；此时 tau 仍只能视为有限时间窗积分。")
        print("  -> Langevin/NVT 势能 slope/half-shift 只检查平稳性，不代表 NVE 能量守恒。")
        print("="*72)
        return
    ...
    for name, cfg in experiments.items():
        ...
        metrics_df.loc[name, "enable_water_response"] = float(cfg["enable_water_response"])
        metrics_df.loc[name, "enable_solute_polarization"] = float(cfg["enable_solute_polarization"])
    metrics_df.to_csv("1ckk_v23_charge_mod_ablation_metrics.csv", index_label="config")
    if energy_rows:
